...
func GetClusterAddress(addrs []string) ([]string, error) {
	...
	for i := 0; i < len(addrs); i++ {
		...
		if err != nil {
			mylog.Error("Connect to redis fail, %s", err.Error())
			continue
		}
		break
	}
	...
	if err != nil {
		mylog.Error("exec [cluster nodes] fail, %s", err.Error())
		return nil, err
	}
	...
	for i := 0; i < len(cluster.addrs); i++ {
		...
		if len(cluster.password) > 0 {
			if _, err := conn.Do("AUTH", cluster.password); err != nil {
				mylog.Error("auth fail by password: %s, addr: %s, err: %s", cluster.password, cluster.addrs[i], err.Error())
				conn.Close()
				continue
			}
		}
...
func (cluster *MyRedisCluster) initCluster(c *redis.Conn) bool {
	...
	ret, err := redis.String(reply, err)
	if err != nil {
		mylog.Error("initCluster fail, %s", err.Error())
		return false
	}
	...
	for i := 0; i < len(lines); i++ {
		...
		addr := fields[1]
		mylog.Info("connect to redis host: %s, password: %s", addr, cluster.password)
		conn, err := redis.Dial(
			"tcp",
			addr,
			redis.DialConnectTimeout(5*time.Second),
		...
		if len(cluster.password) > 0 {
			if _, err := conn.Do("AUTH", cluster.password); err != nil {
				mylog.Error("auth fail by password: %s, addr: %s, err: %s", cluster.password, addr, err.Error())
				cluster.Close()
				return false
			}
		}
...
func (cluster *MyRedisCluster) excute(retry bool, key string, cmd string, args ...interface{}) (interface{}, error) {
	...
	if err != nil {
		if retry {
			mylog.Warn("ucReis do fail,  key:%s, cmd:%s, error:%s", key, cmd, err.Error())
			cluster.reconnect()
			return cluster.excute(false, key, cmd, args...)
		} else {
			mylog.Error("ucReis retry do fail,  key:%s, cmd:%s, error:%s", key, cmd, err.Error())
		}
	}
	return reply, err
}
...
func (cluster *MyRedisCluster) eval(retry bool, key string, script *redis.Script, args ...interface{}) (interface{}, error) {
	...
	if err != nil {
		if retry {
			mylog.Warn("ucReis do fail,  key:%s, cmd:%s, error:%s", key, "eval", err.Error())
			cluster.reconnect()
			return cluster.eval(false, key, script, args...)
		} else {
			mylog.Error("ucReis retry do fail,  key:%s, cmd:%s, error:%s", key, "eval", err.Error())
		}
	}
	return reply, err
}
...
func (cluster *MyRedisCluster) Commit() (reply []UcRedisCommand, err error) {
	...
	if err != nil {
		mylog.Error("redis pipeline commit error:%s", err.Error())
	}
...
func (cluster *MyRedisCluster) commitAll() error {
	...
	for i := 0; i < len(cluster.nodes); i++ {
		if cluster.nodes[i].HasPipeCommand() {
			...
			if err != nil {
				saved_err = err
				mylog.Warn("redis pipeline commit node failed, %s", err.Error())
			}
...
func (cluster *MyRedisCluster) commitNode(index int) (saved_err error) {
	...
	if saved_err != nil {
		mylog.Warn("commitNode flush to server error:%s", saved_err.Error())
		return
	}
	...
	for i := 0; i < len(cmdList); i++ {
		...
		if err != nil {
			...
			mylog.Warn("commitNode receive from server error:%s", saved_err.Error())
			continue
		}
